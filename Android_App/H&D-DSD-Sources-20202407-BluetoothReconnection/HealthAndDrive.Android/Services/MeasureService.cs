...
namespace HealthAndDrive.Droid.Services
{
    ...
    public class MeasureService : Service, IMeasure
    {
        ...
        {
            // Check delay
            if(( Delay >= 900 ) && ( this.MeasureServiceState == MeasureServiceState.WAITING_DATA ))
            {
                // Re init Delay
                Delay = 0;
                ...
                if( this.lastNotificationMeasure != null )
                {
                    ...
                    TimeSpan Temp = DateTime.Now - lastNotificationMeasure.NotificationMeasureDate;
                    double gapTimeInSeconde = Temp.TotalSeconds;

                    // Check the age of the last measure
                    // if most than 15 minutes reconnect the last device
                    if(gapTimeInSeconde >= 900)
                    {
